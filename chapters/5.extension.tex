...
\section{Avoid setting more capabilities than available CPUs} % XXX: specify N?
...
% capability mimics a CPU for the runtime system
% represents the number of Haskell threads that can trully run in parallel
% makes sense to not outnumber the cores
% our study confirms that it degrades performance

% the oficial documentation recomends to use N=CORES but leave opens if virtual cores should count
% our experiments shows that the performance does not improve for more than physical cores
