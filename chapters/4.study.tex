...
In this chapter, we present an empirical study evaluating the performance and energy consumption characteristics of three thread management constructs and three data-sharing primitives. We start by providing a brief overview of the problem (\secref{sec:overview}). \secref{sec:setup} describes the benchmark we developed, the environment we used and the methodology. \secref{sec:results} provides our results. \secref{sec:discussion} presents a discussion about our results. Finally, \secref{sec:threats} list the threats to validity.

\section{Overview}\label{sec:overview}
%% Give more context before presenting the research questions
...
We selected a variety of concurrent Haskell programs to use as benchmarks in our study. Benchmarks \chameneos, \fasta, \knucleotide, \mandelbrot, \regex, and \spectral are from The Computer Language Benchmarks Game\footnote{http://benchmarksgame.alioth.debian.org/} (CLBG). CLBG is a benchmark suite aiming to compare the performance of various programming languages. Benchmark \dining is from  Rosetta Code \footnote{http://rosettacode.org/}, a code repository of solutions to common programming tasks. Benchmarks \tsearch and \warp were developed by us. Table~\ref{tab:benchmarks} presents descriptions for all the benchmarks.

\begin{savenotes}
\begin{table*}[th]
\begin{center}
\caption{The benchmarks employed in this study.}\label{tab:benchmarks}
\newcolumntype{V}{>{\centering\arraybackslash}m{.16\linewidth} }
\begin{tabular}{ | V | m{0.77\linewidth} | }
  \hline
  \textbf{Benchmark} & \multicolumn{1}{c|}{\textbf{Description}} \\

  \hline
  \chameneos        & In this benchmark chameneos creatures go to a meeting place and exchange colors with a meeting partner. It encodes symmetrical cooperation between threads. \\

  \hline
  \fasta            & This benchmark generates random DNA sequences and writes it in FASTA format\footnote{https://en.wikipedia.org/wiki/FASTA\_format}. The size of the generated DNA sequences is in the order of hundreds of megabytes. In this benchmark,  each worker synchronizes with the previous one to output the sub-sequences of DNA in the correct order. \\

  \hline
  \knucleotide      & This benchmark takes a DNA sequence and counts the occurrences and the frequency of nucleotide patterns. This benchmark employs string manipulation and hashtable updates intensively. There is no synchronization in the program besides the main thread waiting for the result of each worker. \\
  ...
  \hline
  \regex            & This benchmark implements a string-based algorithm that performs multiple regular expression operations, match and replace, over a DNA sequence. The only synchronization point is the main thread waiting for the result of each worker. \\

  \hline
  \spectral         & The spectral norm is the maximum singular value of a matrix. Synchronizes workers using a cyclic barrier. \\

  \hline
  \dining           & An implementation of the classical concurrent programming problem posed by Dijkstra. The philosophers perform no work besides manipulating the forks and printing a message when eating. \\

  \hline
  \tsearch          & A parallel text search engine. This benchmark searches for occurrences of a sentence in all text files in a directory and its sub-directories. It is based on a previous empirical study comparing STM and locks~\citep{pankratius:2011}. \\

  \hline
  \warp             & Runs a set of queries against a Warp server retrieving the resulting webpages. Warp is the default web server used by the Haskell Web Application Interface, part of the Yesod Web Framework. This benchmark was inspired by the Tomcat benchmark from DaCapo~\citep{blackburn:2006}. \\
...
In this section, we report the results of our experiments with concurrent Haskell programs. The results are presented in \figref{fig:conc_bench}. Here, the odd rows are energy consumption results, while the even rows are the corresponding running time results. We omitted the experiments using 64 capabilities in order to make the charts more readable. The charts including this configuration as well as all the data and source code used in this study are available at \href{http://green-haskell.github.io/}{green-haskell.github.io}.
\newline

\begin{figure*}[tp]
\caption{Energy/Time with alternative concurrency abstractions and varying the number of capabilities}
\begin{center}
$
\begin{array}{ccc}
 \includegraphics[width=.32\textwidth]{images/conc_bench/chameneos-redux-energy} &
 \includegraphics[width=.32\textwidth]{images/conc_bench/dining-philosophers-energy} &
 \includegraphics[width=.32\textwidth]{images/conc_bench/fasta-energy} \\

 \includegraphics[width=.32\textwidth]{images/conc_bench/chameneos-redux-time} &
 \includegraphics[width=.32\textwidth]{images/conc_bench/dining-philosophers-time} &
 \includegraphics[width=.32\textwidth]{images/conc_bench/fasta-time} \\
 \\ %\\

 \includegraphics[width=.32\textwidth]{images/conc_bench/k-nucleotide-energy} &
 \includegraphics[width=.32\textwidth]{images/conc_bench/mandelbrot-energy} &
 \includegraphics[width=.32\textwidth]{images/conc_bench/regex-dna-energy} \\


 \includegraphics[width=.32\textwidth]{images/conc_bench/k-nucleotide-time} &
 \includegraphics[width=.32\textwidth]{images/conc_bench/mandelbrot-time} &
 \includegraphics[width=.32\textwidth]{images/conc_bench/regex-dna-time} \\
 \\ %\\

 \includegraphics[width=.32\textwidth]{images/conc_bench/spectral-norm-energy} &
 \includegraphics[width=.32\textwidth]{images/conc_bench/tsearch-energy} &
 \includegraphics[width=.32\textwidth]{images/conc_bench/warp-energy} \\

 \includegraphics[width=.32\textwidth]{images/conc_bench/spectral-norm-time} &
 \includegraphics[width=.32\textwidth]{images/conc_bench/tsearch-time} &
 \includegraphics[width=.32\textwidth]{images/conc_bench/warp-time} \\
\end{array}
$
\end{center}
\label{fig:conc_bench}
\end{figure*}
...
%Generally, especially in the sequential benchmarks, high performance is a proxy for low energy consumption. Our first study (Section~\ref{sec:datastructures}) highlighted this for a number of different data structure implementations and operations. Concurrency makes the relationship between performance and energy less obvious, however. Also, there are clear benefits in employing  different thread management constructs and data-sharing primitives. This section examines this in more detail.
...
Although there is no overall winner, there is a more or less clear loser, when thread management construct have a strong impact on energy: \forkOS. In only one of the benchmarks the \forkOS variants did not have the worst energy consumption and worst performance: \regex. According to the Haskell documentation~\footnote{https://hackage.haskell.org/package/base-4.8.1.0/docs/Control-Concurrent.html\#v:forkOS}, {\em ``Using forkOS instead of forkIO makes no difference at all to the scheduling behavior of the Haskell runtime system''.} If that is the case, the extra time and energy consumption stem from the need to switch OS threads to execute work passed to a call to \forkOS. This overhead does not exist for \forkOn and \forkIO.
